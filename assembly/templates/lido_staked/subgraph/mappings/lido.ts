...
import { WITHDRAWAL_QUEUE_ADDRESS } from '../utils/constants'
import { LidoUserPosition } from '../../generated/schema'

export function handleSubmit(event: Submitted): void {
  // Ids
  const userId = event.params.sender.toHexString()
  const positionId = userId + '-' + '0'
...
export function handleTransfer(event: Transfer): void {
  if (
    event.params.to == WITHDRAWAL_QUEUE_ADDRESS ||
